...
int main(int argc, char **argv)
{
    ...
    AccountUid uid={0};
    u64 application_id=0x0100F2C0115B6000;//ApplicationId of the save to mount, in this case TOTK.
    ...
    if (R_SUCCEEDED(rc)) {
        ...
        else
        {
            ...
            // fp2 = fopen("save:/TEST/directory.txt", "a"); - uncomment to write to file
            printf("Dir-listing for 'save:/':\n");
            while ((ent = readdir(dir)))
            {
                printf("d_name: %s\n", ent->d_name);
                // fprintf(fp2, "d_name: %s\n", ent->d_name); - uncomment to write to file
            }
            // fclose(fp2);  - uncomment to write to file
            closedir(dir);
            printf("Done.\n");
        }
